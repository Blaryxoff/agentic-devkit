...
1. **[overview.md](conduct/overview.md)** — technology stack, reading order, how to use with AI agents
2. **[architecture.md](conduct/architecture.md)** — layered architecture, domain boundaries, services/actions, contracts, project folder structure
3. **[php.md](conduct/php.md)** — naming, typing, class design, error handling, framework integration
4. **[configs.md](conduct/configs.md)** — Laravel config files, env-driven configuration, validation, and defaults
5. **[cmd.md](conduct/cmd.md)** — Artisan command structure, dependency wiring, scheduling, queue workers, graceful shutdown
6. **[error_handling.md](conduct/error_handling.md)** — exceptions, domain/infrastructure error boundaries, wrapping/reporting rules
7. **[logging.md](conduct/logging.md)** — Laravel logging channels, structured context, log levels, what to log/not log
8. **[observability.md](conduct/observability.md)** — tracing/metrics strategy, request lifecycle visibility, logs-to-traces correlation
9. **[dependencies.md](conduct/dependencies.md)** — approved third-party libraries by concern, forbidden packages
...
- **[security.md](conduct/security.md)** — input validation, SQL injection, CSRF, CORS, rate limiting
- **[makefile.md](conduct/makefile.md)** — standard project task targets
- **[anti_patterns.md](conduct/anti_patterns.md)** — common mistakes with code examples
- **[code_review_checklist.md](conduct/code_review_checklist.md)** — pre-commit/PR review checklist
...
4. **Review** — validate with [code review checklist](conduct/code_review_checklist.md)
5. **Commit** — follow [git conventions](conduct/git.md): Conventional Commits format, one logical change per commit, run project pre-commit checks
...
3. Update [code_review_checklist.md](conduct/code_review_checklist.md) if the change adds checkable items
4. Create a PR
