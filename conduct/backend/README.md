...
- Feed to your LLM:
  - Created spec
  - Expected test cases
  - [Code conduct rules](./conduct/)
  - [Testing rules](./testing/)
- Validate the newly created test cases; make sure all edge cases and happy paths are covered
- Review the code (or by using [checklist](./conduct/code_review_checklist.md) only or all files)
- Commit following [git conventions](./conduct/git.md): Conventional Commits format, one logical change per commit, run project checks (at minimum `vendor/bin/pint --dirty`, related `php artisan test`, and relevant frontend lint/format checks)
...
1. Create your changes. Make sure that (**ALWAYS** use an LLM to validate the following rules):
   ...
   - [code_review_checklist.md](./conduct/code_review_checklist.md) (contains only core topics; used in pre-commit hooks)
2. Create a PR
