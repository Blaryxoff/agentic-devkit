...
2. **Preflight.** `browser-qa.md` §2.
3. **Ground the scope.** Read feature docs under `docs/plans/`; read `.devkit/toolkit.json`. `inputs-grounding-gate.md`.
4. **Map the QA surface.** `browser-qa.md` §4.
5. **Seed strategy.** `browser-qa.md` §3 — record exact commands in the plan.
6. **Build the scenario matrix.** `browser-qa.md` §5 — roles × pages × viewports × lifecycle × validation × interaction depth × propagation × permissions × regression. This becomes the task list.
...
### Product plan
QA objective, scope/non-scope, acceptance = "every matrix scenario in `browser-qa.md` §5 executed and every finding appended to the report as it is found".

### Dev (ralphex) plan — tasks as browser actions using chrome-devtools MCP tool names from `browser-qa.md` §8
...
- **Task 2 — Unauthed sweep of protected routes:** `browser-qa.md` §5.1 per route; append findings.
- **Task 3 — Unauthed sweep of public routes:** `browser-qa.md` §5.2 per route; append findings.
- **Tasks 4…N — per role:** login (`browser-qa.md` §5.3); per page × viewport (`§5.4`): lifecycle (`§5.5`), validation (`§5.6`), interaction depth (`§5.7`), console/network (`§5.11`).
- **Cross-role access-propagation tasks:** `browser-qa.md` §5.8 per controllable section.
- **Permission-matrix tasks:** `browser-qa.md` §5.9.
- **Regression tasks:** `browser-qa.md` §5.10.
- **Figma tasks** (when URLs in scope): `browser-qa.md` §5.12 per linked frame × viewport.
...
- Finding format: `browser-qa.md` §7. Screenshots under `docs/qa/screenshots/`.
