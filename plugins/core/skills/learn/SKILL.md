...
- stable architecture boundaries, ownership, and module layout — never an inventory of which files are live or dead.
  A call chain qualifies when it is the module's durable entry contract, not when this session enumerated it while
  chasing a defect or a dead sibling.
...
- required integration, testing, build, deployment, or operational workflows not obvious from their canonical files,
  and made binding by the project's own configuration or a team decision rather than by one failed run
...
- actionable repository findings — dead or unreferenced code, stale configuration, cleanup, defects left unfixed

When a candidate matches both an accept and a reject bullet, reject it. Labelling a tooling failure a "workflow" does
not make it one.
...
3. **Established-evidence test** — is it supported beyond one failure path by multiple project locations, an existing
   workflow, or a confirmed team decision? Repeat sightings of one failure, and agreement between reviewers or models,
   are confidence in the observation, not evidence of intent.
...
5. **Placement test** — is a project instruction file the best enforcement point, rather than code, tests, types, or
   canonical technical documentation — including documentation the project should have and has not written yet?
6. **Survives-its-own-fix test** — does it stay true after the project acts on it? If acting on the finding makes it
   false, it is work, not memory.
...
- **Not project memory** — an actionable repository finding goes to a fix, or to user-approved deferral via
  `Skill(devkit-core--backlog)` (Codex/Cursor: `devkit-backlog`), which applies
  `plugins/core/conduct/deferred-work-backlog.md` and its default-branch gate. A startup command,
  environment-variable relationship, or
  troubleshooting step belongs in the project's development documentation, or in automation that removes the failure.

**Default only a qualified candidate's destination ambiguity to the active harness's team-shared project instruction
file.** Never use this default to rescue a candidate that failed qualification. Never create or update
...
4. **Route actionable repository findings** per `plugins/core/conduct/learning-capture-gate.md` → "Terminal action".
   That branch owns the offer and its bounds. Never write one as memory, and never file one on your own initiative.
5. **Classify each discovery** to its destination per the rules above.
6. **Limit and consolidate** — keep at most three high-value discoveries. Prefer merging or replacing an existing entry over appending overlapping text.
7. **Present** the discoveries, each tagged with its inferred destination:
   ```markdown
   ## [Section] → project AGENTS.md
   - Discovery 1
   ```
8. **Confirm with the user** — use the environment's structured question tool when available; otherwise ask a concise
