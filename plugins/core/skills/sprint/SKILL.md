...
name: devkit-sprint
description: >-
  audit and update sprint-planning Google Sheets workbooks: inspect every visible and hidden sprint sheet, linked task specifications, Git branches and commits, and scoped agent sessions; estimate remaining coding hours, classify back/front/common/full work, assign executors, balance real coding capacity, and add native task-specific Drive smart chips. Use for sprint estimation, capacity allocation, executor assignment, or repairing planning-workbook links and totals. Does not implement product code.
---
...
Codex; use the fastest available research subagent on another harness and report the substitution. Every lane must cover
its entire assigned source set, including inaccessible-item reporting; sampling is not sufficient.

| Lane | Required coverage | Output |
|---|---|---|
| Workbook history | Every worksheet, including `hidden`/`veryHidden`, hidden rows, every sprint-like table and capacity block, formulas, validation, estimates, directions, executors, priorities, task IDs, links, and duplicate/carry-over rows | Normalized history ledger, estimate distributions, capacity/formula defects, ownership continuity |
| Linked specifications | Every unique task/spec URL from all sprint sheets; preserve Docs `tab`, `#heading`/bookmark, Sheets `gid`, and Figma `node-id`; inspect the complete accessible specification | Row-to-exact-link map, completeness/confidence, roles, surfaces, state transitions, integration and QA scope |
| Git and sessions | All relevant repository refs and worktrees, non-merge commits, diffs, normalized author identities, and repo-scoped Claude/Codex/Cursor sessions | Task-to-code evidence, direction, owner continuity, complexity signals, ambiguous/unproven matches |
...
| Code | repos/refs, implementation commits, changed layers, tests/review/QA evidence |
...
1. Estimate the epic first, then allocate its shared implementation to task rows. Do not charge every row for the same
   model, API, component, test setup, review, or QA foundation.
2. Estimate remaining real coding work: implementation, eligible tests, review fixes, and focused QA. Keep meetings,
   coordination, release work, and already-completed work separate when the workbook models them separately.
3. Prefer completed analogues with the same task ID or scope. Weight exact scope and recency above repository-wide
   frequency. Use the workbook's observed estimate increments instead of inventing a new scale.
4. Use Git/session evidence to understand scope and continuity, not as a stopwatch. Never sum parallel subagent time,
   review waits, merge commits, generated lines, or raw LOC into developer hours.
5. Mark incomplete specifications as provisional. Never turn a blank or unknown estimate into zero.
6. Keep large tasks honest. Do not shrink estimates merely to make totals equal capacity.
...
- every worksheet and hidden row was included in the evidence inventory;
