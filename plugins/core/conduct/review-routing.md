...
| "test", "протестируй", "QA", "smoke-test", "прокликай" | running app / UI | `devkit-qa-tester` (or `devkit-qa-tester-ralphex` when a persisted plan + markdown report is wanted) |
...
- **Test ≠ review:** "test/QA/протестируй" never means a static reviewer — it means `devkit-qa-tester` (drives the running app).
