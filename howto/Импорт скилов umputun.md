...
| `devkit-learn` | «learn», «update agents.md/claude.md» или терминальный learning-capture gate — предложить до трёх подтверждённых проектных контрактов для нативного файла инструкций текущего агента (`AGENTS.md`, `CLAUDE.md` или принятого аналога); не сохранять детали решённой задачи и технические gotcha; реальную работу по репозиторию уводить в фикс или бэклог, а не в память; запись только после выбора пользователя. |
| `devkit-backlog` | `docs/backlog/`: список, разбор, фикс, удаление или добавление принятых находок; гейт дефолтной ветки, дедуп, без авто-коммита. |
...
- `learning-capture-gate.md` — терминальный фильтр для подтверждённых проектных контрактов, полезных в других будущих
  задачах; детали решённой задачи и технические gotcha отклоняет и не читает проектную память. Три исхода: память,
  однострочное предложение «починить сейчас или отложить в бэклог» для реальной работы по репозиторию (мёртвый код,
  устаревший конфиг, неисправленный дефект), либо молчание.
- `deferred-work-backlog.md` — контракт отложенной работы в `docs/backlog/` (по файлу на пункт, `worth`/`where`/`added`,
  гейт дефолтной ветки); исполняется скилом `devkit-backlog`.
