...
| `devkit-learn` | «learn», «update agents.md/claude.md» или терминальный learning-capture gate — предложить до трёх подтверждённых проектных контрактов для нативного файла инструкций текущего агента (`AGENTS.md`, `CLAUDE.md` или принятого аналога); не сохранять детали решённой задачи и технические gotcha; запись только после выбора пользователя. |
...
- `learning-capture-gate.md` — терминальный фильтр для подтверждённых проектных контрактов, полезных в других будущих
  задачах; детали решённой задачи и технические gotcha отклоняет, без кандидата завершается молча и не читает проектную
  память.
