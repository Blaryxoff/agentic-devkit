...
# devkit_frontmatter <skill_file>
# Print only the YAML frontmatter — the lines between the first two `---` delimiters.
# LC_ALL=C keeps this on byte semantics: macOS one-true-awk aborts on multibyte input
# (see docs/backlog/awk-crashes-on-multibyte-skill-descriptions.md), and every pattern
# here is ASCII-anchored, so no character is ever split.
devkit_frontmatter() {
  LC_ALL=C awk '
    NR == 1 && $0 !~ /^---[[:space:]]*$/ { exit }
    NR == 1 { next }
...
# devkit_is_subagent <skill_file>
# True when `claudeSubagent: true` appears in the frontmatter — never in the body,
# where skill-creator documents the same key inside a fenced example.
devkit_is_subagent() {
  devkit_frontmatter "$1" | LC_ALL=C grep -qE "^claudeSubagent:[[:space:]]*true[[:space:]]*(#.*)?$"
}
...
emit_subagent() {
  ...
  awk -v sentinel="$DEVKIT_AGENT_SENTINEL" -v out_dir="$agents_dir" '
    ...
    END {
      ...
      printf "---\n" > out
      printf "name: %s\n", name >> out
      # A source description already written as a quoted scalar round-trips verbatim.
...
generate_subagents() {
  ...
  while IFS= read -r name; do
    ...
    for skill_md in "$skills_dir"/*/SKILL.md; do
      ...
      local fm_name
      fm_name=$(devkit_frontmatter "$skill_md" | LC_ALL=C grep -m1 '^name:' | sed 's/^name:[[:space:]]*//;s/[[:space:]]*$//')
