...
for rule in (
    ...
    "nothing numeric precedes the table, and no number in it is restated in the prose below",
    ...
    "developer hours are what the lane costs its human",
    "### 5. calculate developer time, then elapsed",
    "total developer time across the lanes as one figure in hours",
    "lane developer hours sum; no other column does",
    "unattended agent, test, and ci runtime is never developer time",
    ...
    "developer time is the first column and the figure the recommendation is stated in",
    ...
    "inputs with no source in the system",
    "work on another party's side, whose date you do not set",
    "report a level only when you can name the work it adds over the level below",
    ...
    "name the single lane that accounts for any excess over the anchor's recorded span",
